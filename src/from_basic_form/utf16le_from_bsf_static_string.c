...
static size_t utf16le_char_from_unicode(uint32_t unicode, uint8_t* bytes) {
    if (unicode < 0x10000) {
        // assert(unicode == unicode & 0b1111_1111_1111_1111)
        // bytes[0] = unicode[0..8]
        // bytes[1] = unicode[8..16]
        bytes[0] = unicode & 0b11111111;
        bytes[1] = unicode >> 8;
        return 2;
    } else {
        ...
        // bytes[0] = first_word[0..8]
        // bytes[1] = first_word[8..16]
        // bytes[2] = second_word[0..8]
        // bytes[3] = second_word[8..16]

        bytes[0] = first_word & 0b11111111;
        bytes[1] = first_word >> 8;
        bytes[2] = second_word & 0b11111111;
        bytes[3] = second_word >> 8;

        return 4;
    }
}

struct char_slice_s utf16le_from_bsf_static_string(struct bsf_static_string_s static_string) {
    ...
    while (howfar) {
        ...
        size_t written_count = utf16le_char_from_unicode(unicode, bytes);
