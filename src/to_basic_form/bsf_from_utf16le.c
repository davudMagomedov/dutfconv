...
static uint16_t read_16word_le(struct char_iterator_s* iterator) {
    ...
    uint16_t word =
        (uint16_t)char_iterator_next(iterator) + ((uint16_t)char_iterator_next(iterator) << 8);

    return word;
}

static uint32_t read_unicode_utf16le(struct char_iterator_s* iterator) {
    uint16_t leading, trailing;

    leading = read_16word_le(iterator);
    ...
    trailing = read_16word_le(iterator);
...
struct bsf_static_string_s bsf_from_utf16le(struct char_iterator_s iterator) {
    struct bsf_buffer_s buffer = bsf_buffer_init();

    while (iterator.howfar) {
        uint32_t unicode = read_unicode_utf16le(&iterator);
